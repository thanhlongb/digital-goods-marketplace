...
const getSearchOrderByRatings = async (where, order, limit, pageIdx) => {
  const res = await models.review.findAndCountAll({
    ...
    order: [[sequelize.literal("rating_avg " + order[1])]],
    offset: pageIdx,
    limit: limit,
    raw: true,
  });
...
const getSearchOrderBySales = async (where, order, limit, pageIdx) => {
  const res = await models.purchase.findAndCountAll({
    ...
    group: ["productId", "product.id"],
    order: [[sequelize.literal("total_sales " + order[1])]],
    offset: pageIdx,
    limit: limit,
    raw: true,
  });
...
const getTopSellers = async (limit_str) => {
    ...
    const row = await sequelize.query(
      `SELECT PRODUCT.*, PURCHASES.SALES FROM PRODUCT,(SELECT "purchase"."productId", COUNT("productId") AS SALES FROM PURCHASE GROUP BY "productId") AS PURCHASES WHERE PRODUCT.ID = "purchases"."productId" ORDER BY PURCHASES.SALES DESC LIMIT ${limit}`,
      {
        type: QueryTypes.SELECT,
        plain: true
      }
